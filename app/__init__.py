...
def create_app(config_class=Config):
    ...
    with app.app_context():
        ...
        # Migration: Add new columns for features 1, 8, 9, 10
        try:
            with db.engine.connect() as conn:
                conn.execute(text("ALTER TABLE student ADD COLUMN password_hash VARCHAR(60)"))
                conn.commit()
            app.logger.info("Migration: 'password_hash' column added to student table.")
        except Exception:
            pass
        ...
        try:
            with db.engine.connect() as conn:
                ...
                conn.commit()
            app.logger.info("Migration: 'is_deleted' column added to student table.")
        except Exception:
            pass

        try:
            with db.engine.connect() as conn:
                conn.execute(text("ALTER TABLE user ADD COLUMN show_name_on_subject BOOLEAN DEFAULT 0"))
                conn.commit()
            app.logger.info("Migration: 'show_name_on_subject' column added to user table.")
        except Exception:
            pass

        try:
            with db.engine.connect() as conn:
                conn.execute(text("ALTER TABLE site_info ADD COLUMN content_ku TEXT"))
                conn.commit()
            app.logger.info("Migration: 'content_ku' column added to site_info table.")
        except Exception:
            pass
